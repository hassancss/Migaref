...
public function getmanageprizeAction() {
        ...
        try {
          ...
          foreach ($redeemprizes as $key => $value) {
            ...
            $action='<select class="input-flat" id="change_redeemstatus" onchange="chnageRedeemstatus(this)">';
              ...
              $action.='<option  value="'.$value['migarefrence_ledger_id'].'@1@'.$value['customer_id']."@".$value['prize_id']."@".$value['credits_number']."@".$value['prize_name']."@".$value['ledger_id'].'">'.__("Delivered").'</option>';
              $action.='<option  value="'.$value['migarefrence_ledger_id'].'@2@'.$value['customer_id']."@".$value['prize_id']."@".$value['credits_number']."@".$value['prize_name']."@".$value['ledger_id'].'">'.__("Refused").'</option>';
            $action.='</select>';
              $report_collection[]=[
                        $count++,
  ...
  public function saveprizenotificationAction(){
      if ($datas = $this->getRequest()->getPost()) {
          try {
              ...
              // Refreal validation
              if (empty($datas['type'])) {
                $errors .= __('Please select notification type.') . "<br/>";
              }
      ...
      if ($datas = $this->getRequest()->getPost()) {
          try {
              ...
              if (!empty($errors)) {
                  ...
                  $short_link   = $this->urlshortAction($long_url,$bitly_crede[0]['bitly_login'],$bitly_crede[0]['bitly_key']);                  
              }
              $html = [
                'success'         => true,
                'message'         => __('Successfully data saved'),
  ...
  public function saveautomationreminderformAction(){
      if ($datas = $this->getRequest()->getPost()) {
          try {
              ...
              $autoReminderResponse = Migareference_Model_Db_Table_Migareference::TestautomationTriggerscron();              
              $errors        = "";
              $trigger_status= "";
  ...
  public function urlshortAction($long_url = "", $login = "", $genericAccessToken = "") {
    if ($long_url) {
        ...
        if ($http_status !== 200) { // 201 is the expected status for successful creation
            // Log error if necessary
            error_log('Bitly API Error: HTTP ' . $http_status . ' Response: ' . $bitlyresult);
            return $long_url;
        }

        $bitlyresult = json_decode($bitlyresult);

        if (isset($bitlyresult->link)) {
            return str_replace('http:', 'https:', $bitlyresult->link);
        } else {
            // Log error if necessary
            error_log('Bitly API Response Error: ' . json_encode($bitlyresult));
            return $long_url;
        }
    }

    return $long_url; // Return long URL if no input provided
}
  ...
  public function saveprereportAction(){
    if ($data = $this->getRequest()->getPost()) {
        try {
              ...
              // Bitly Validations
              if (!empty($data['bitly_login']) && !empty($data['bitly_key']) && $data['bitly_key']!='Sales Expectation' && $data['bitly_login']!='Sales Expectation') {
                $short_link       = $this->urlshortAction("https://www.google.com",$data['bitly_login'],$data['bitly_key']);
                if ($short_link!="https://www.google.com") {
                  true;
                }else {
                  $errors .= __('Could not connect to Bitly API.Please try again later.')."<br/>";
                }
              }
              if(!empty($errors)) {
                  throw new Exception($errors);
  ...
  public function updateredeemstatusAction() {
      if ($key = $this->getRequest()->getParam('key')) {
          try {
                ...
                $priz_id       = $temp_keys[3];
                $credits       = $temp_keys[4];
                $prize_name    = $temp_keys[5];
                $ledger_id     = $temp_keys[6];
                $earning['app_id']           = $app_id;
                $earning['user_id']          = $referrer_id;
  ...
  public function AfieldbykeyAction(){
    $build_index   = $this->getRequest()->getParam('build_index');
    $migareference  = new Migareference_Model_Migareference();
    $linkids       = explode("*",$build_index);
    foreach ($linkids as $key => $value) {
  ...
  public function genratreportsdbAction() {      
          try {
              ...
              foreach ($prospect_jobs as $key => $value) {                
                ...
                // if ($pre_settings[0]['enable_report_type']==1) {   
                //   $mobile[]=$value['report_custom_type'];
                // }
          ...
          try {
                ...
                foreach ($field_data as $keyy => $valuee) {                
                  if ($valuee['type']==1 && $valuee['is_visible']==1) { //for static fields we will mask some understandable tags like nome to owner_name
                    ...
                    // if ($static_fields[$valuee['field_type_count']]['name']=='note') {
                    //   $tags_list.='owner_email, ';
                    // }else{
                    //   $tags_list.=$tag;
                    // }
                    // $tags_list.=str_replace(' ', '_', strtolower($valuee['label'])).", ";
                  }elseif($valuee['is_visible']==1) {
      ...
      if ($report_id= $this->getRequest()->getParam('report_id')) {
          try {
            ...
            if ($reportitem[0]['consent_bitly']=='') {
              ...
              $consent_link       = $this->urlshortAction($consent_link,$bitly_crede[0]['bitly_login'],$bitly_crede[0]['bitly_key']);
              $datas['app_id']									= $app_id;
