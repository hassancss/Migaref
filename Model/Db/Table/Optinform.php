...
class Migareference_Model_Db_Table_Optinform extends Core_Model_Db_Table
{
        ...
        if (empty($res_option)) {
            $defaultValues = [
                'app_id' => $app_id,
                'button_width' => 100,
                'button_label' => 'SI! Voglio candidarmi',
                'button_text_color' => '#ffffff',
                'button_background_color' => '#000000',
                'confirmation_message' => 'Complimenti, Candidatura Ricevuta!',
                'created_at' => date('Y-m-d H:i:s'),
            ];

            $this->_db->insert($this->_name, $defaultValues);
            $defaultValues['migareference_optin_form_id'] = $this->_db->lastInsertId();

            $res_option[] = $defaultValues;
        }
