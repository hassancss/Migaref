<?php
class Migareference_Mobile_AdminstatsController extends Application_Controller_Mobile_Default
{    
    ...
    public function agenttotalreportsAction()
    {
      if ($data = $this->getRequest()->getQuery()) {
        ...
        if ($sponsor_id==-1) {
            ...
            $referrers       = $referrerstats->agentReferrersTblAgent($app_id,$sponsor_id);                                                
        }
        $ajax_responce = [];
        $fin_res = [];
        if (!empty($referrers)) {
